// Team Pages
export { TeamPage } from './teams'
export { BackendTeamPage } from './teams/team/Backend'
export { EducationTeamPage } from './teams/team/Education'
export { EventTeamPage } from './teams/team/Event'
export { FinanceTeamPage } from './teams/team/Finance'
export { FrontendTeamPage } from './teams/team/Frontend'
export { MarketingTeamPage } from './teams/team/Marketing'
export { OutreachTeamPage } from './teams/team/Outreach'
export { ProjectTeamPage } from './teams/team/Project'

// About Page
export { AboutPage } from './about'

// Error Page
export { ErrorPage } from './error'
